...
public class JavaKeyStoreProvider implements KeyProvider {
  ...
  public Key getKey(String name) throws IOException {
    try {
      readLock.lock();
      java.security.Key javaKey = store.getKey(name, getAliasPassword(name));
      Key key = new Key(name, javaKey.getEncoded());
      return key;
    } catch (UnrecoverableKeyException e) {
      throw new IOException("Can not recover key " + name);
    } catch (KeyStoreException e) {
      throw new IOException("Can not get key " + name);
    } catch (NoSuchAlgorithmException e) {
      throw new IOException("Can not get algorithm for key " + name);
    } finally {
      readLock.unlock();
    }
  }
  ...
  @Override
  public Key[] getKeys(String[] aliases) throws IOException {
  ...
  public Key createKey(String name,
    ...
    try {
      ...
      try {
        store.setKeyEntry(name, new SecretKeySpec(material, cipher),
            password, null);
      } catch (KeyStoreException e) {
        throw new IOException("Can't store key " + name + " in " + this, e);
      }
      return new Key(name, material);
    } finally {
      writeLock.unlock();
    }
  }
